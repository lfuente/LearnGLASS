/////////////////////////////////////////////////////////////
//////////Dynarch Calendar -- JSCal2, version 1.9///////////
///////////////////////////////////////////////////////////
...
/////////////////////////Filter Objet//////////////////////////////
//////////////////////////////////////////////////////////////////
var DateRange = function(c,d1,d2,t1,t2,lang){
    this.dateMin = new Date(t1);//init date
    this.dateMax = new Date(t2);//end date
    this.languaje = new String(lang);//Español,English
    
    //Internal variables,
    //t1, t2 Allow start as string format as date format
    var t1 = new Date(t1);//init date for initial propouse
    var t2 = new Date(t2);//init date for initial propouse
    var created = "no";
    var container = c;
    ...
    var f_btn3 = "f_btn3";
    var text_day = "day";
    var text_period = "period"; 
    var text_reset = "reset";
    var text_From = "From ";
    var text_to = " to ";
    
      
    function set_extremes(a,b){
        //Si existe una funcion llamada temporal_filter_handle
        if(typeof window.temporal_filter_handler == 'function') { 
            temporal_filter_handler(a,b);
        }
        else{       
            alert("the function temporal_filter_handle() doesn't exti. Create it to handle the extremes");
        }
    }
    
    print_temporal_filter();
    function print_temporal_filter(){        
        var input1 = document.createElement('input');
        input1.type = 'text';
        input1.size = input_size;
        input1.id = f_date1;
        input1.value = t1.getFullYear()+"-"+(t1.getMonth()+1)+"-"+t1.getDate();
        
        var input2 = document.createElement('input');
        input2.type = 'text';
        input2.size = input_size;
        input2.id = f_date2;
        input2.value =  t2.getFullYear()+"-"+(t2.getMonth()+1)+"-"+t2.getDate(); 
        
        var input3 = document.createElement('input');
        input3.type = 'button';
        input3.value = text_day;
        input3.id = f_btn1;
        
        var input4 = document.createElement('input');
        input4.type = 'button';
        input4.value = text_period;
        input4.id = f_btn2;
        
        var input5 = document.createElement('input');
        input5.type = 'button';
        input5.value = text_reset;
        input5.id = f_btn3;
        ...
        label1.innerHTML = text_From;
        
        var label2 = document.createElement('label');
        label2.innerHTML = text_to;                    
    
        //Create the data container
        var div_father = document.createElement('div');
        div_father.appendChild(label1);    
        div_father.appendChild(input1);
        div_father.appendChild(label2);    
        div_father.appendChild(input2);
        div_father.appendChild(input3);
        div_father.appendChild(input4);
        div_father.appendChild(input5);
    
        a = document.getElementById(container);
        a.innerHTML = "";//delete duplications
        a.appendChild(div_father);  
        
        this.languaje = lang;
        Calendar.setLanguage(this.languaje);
        Calendar.setup({
            inputField : f_date1,
            trigger    : f_btn1,
            fdow          : 1,
    ...
    function reset(){
        document.getElementById(f_date1).value =  t1.getFullYear()+"-"+(t1.getMonth()+1)+"-"+t1.getDate();
        document.getElementById(f_date2).value =  t2.getFullYear()+"-"+(t2.getMonth()+1)+"-"+t2.getDate();
        dateMax = dateMin = null;
        var a = new Date("0");
        var b = new Date();
        set_extremes(a,b);
    }
    ...
    function getSelectionHandler() 
    {
      ...
      return function(cal) 
      {
          ...
          //alert(selectionObject.print("%Y-%m-%d")); //print date
          ...
          else 
          {
              ...
              //Print dates into fields
              dateMin = Calendar.intToDate(startDate);
              dateMax = Calendar.intToDate(selectedDate);
              document.getElementById(f_date1).value = Calendar.printDate(dateMin,dateFormat);
              document.getElementById(f_date2).value = Calendar.printDate(dateMax,dateFormat);
              set_extremes(dateMin,dateMax);
    ...
    function selectHandle() 
    {
        return function(cal) 
        {
            ...
            //Print dates into fields
            dateMax = Calendar.intToDate(selectedDate);
            dateMin = Calendar.intToDate(selectedDate);
